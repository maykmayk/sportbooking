...
export const Landing = () => {
  ...
  const { levelRange, filter, appliedAt } = useFilters();
  ...
  // ricarica: al mount, al cambio giorno, e quando l’utente preme "Applica" (appliedAt)
  useEffect(() => {
    fetchSlots();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedOffset, appliedAt]);

  const fetchSlots = async () => {
    setLoading(true);
    try {
      const minLevel = Number(levelRange[0]).toFixed(1);
      const maxLevel = Number(levelRange[1]).toFixed(1);

      const url =
        Number(selectedOffset) === 0
          ? `/PublicMatches/current?minLevel=${minLevel}&maxLevel=${maxLevel}&filter=${filter}`
          : `/PublicMatches/offset/${selectedOffset}?minLevel=${minLevel}&maxLevel=${maxLevel}&filter=${filter}`;

      const response = await api.get(url);
      const data = response.data;

      const parsedSlots = data.map(item => {
        const localHour = String(item.localHour).padStart(2, "0");
        const localMinute = String(item.localMinute).padStart(2, "0");
        const serverHour = String(item.serverHour).padStart(2, "0");
        const serverMinute = String(item.serverMinute).padStart(2, "0");
        return {
    ...
    <BaseAppLayout>
      <div className="bg-white min-h-screen flex flex-col">
        <div className="w-full h-48 overflow-hidden relative">
          <img src={randomImage} alt="Padel background" className="object-contain" />
        </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {loading ? (
                ...
                slots.map((slot, idx) => (
                  <div key={idx}>
                    {slot.count === 0 ? (
                      <div className="relative rounded-lg text-center py-3 font-bold text-sm bg-gray-200 text-gray-400 cursor-not-allowed">
                        {slot.time}
                      </div>
                    ) : (
                      <Link to={`/pitch-detail/${selectedOffset}/${slot.serverHour}/${slot.serverMinute}?offset=${selectedOffset}&selectedOffset=${selectedOffset}`}>
                        <div className="relative rounded-lg text-center py-3 font-bold text-sm bg-white text-black border border-gray-300 hover:bg-accent hover:text-white cursor-pointer hover:border-accent">
                          {slot.time}
                          <div className="absolute -top-2 -right-2 text-white w-5 h-5 text-xs px-1 flex items-center justify-center rounded-full bg-accent">
                            {slot.count}
                          </div>
                        </div>
                      </Link>
