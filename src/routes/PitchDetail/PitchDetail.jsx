...
export const PitchDetail = () => {
  ...
  // ricarica: al mount, al cambio slot OR quando l’utente preme "Applica"
  useEffect(() => {
    fetchMatches();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [daysOffset, hour, minute, appliedAt]);
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      const url = `/PublicMatches/hour/${hour}/minute/${minute}/daysOffset/${daysOffset}?minLevel=${minLevel}&maxLevel=${maxLevel}&filter=${filter}`;
      ...
      const parsed = data.map((match) => {
        ...
        const isJoinable = players.length < 4;

        return {
          title: match.sportField || "",
          matchType: match.data?.type || "",
          odds: match.data?.level || "",
          teamA: "Team A",
          teamB: "Team B",
          players,
      ...
      setMatches(parsed.sort((a, b) => (a.hasOpenSlots === b.hasOpenSlots ? 0 : a.hasOpenSlots ? -1 : 1)));
    } catch (error) {
